...
const Card = () => {
  const calculateDepartmentStats = () => {
    ...
    const departmentChange = Math.round(
      ((totalDepartments - lastMonthDepartments) / lastMonthDepartments) * 100
    );
    ...
    const staffChange = Math.round(
      ((totalStaff - lastMonthStaff) / lastMonthStaff) * 100
    );

    const totalServices = departmentList.reduce(
      (sum, dept) => sum + dept.services,
      0
    );
    const lastMonthServices = totalServices - 12;
    const servicesChange = Math.round(
  ...
  const data = [
    {
      ...
      progressWidth: `${calculateProgressWidth(
    ...
    },
    {
      ...
      subtitle: `${(
        (stats.departments.active / stats.departments.total) *
        100
      ).toFixed(0)}% of total`,
      progressColor: "bg-gray-800",
      progressWidth: `${
        (stats.departments.active / stats.departments.total) * 100
      }%`,
      isMoney: false,
